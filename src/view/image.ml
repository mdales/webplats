...
let render_image page filename scale (max_width, max_height) =
  match scale with
  | Fit -> render_image_fit page filename (max_width, max_height)
  | Fill -> render_image_fill page filename (max_width, max_height)

let render_thumbnail page thumbnail_size =
  match Page.titleimage page with
  | None -> failwith "blah"
  | Some titleimg ->
      render_image_fill page titleimg.filename (thumbnail_size, thumbnail_size)
