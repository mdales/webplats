...
let render_atom base_section site pages =
  ...
  let author = match (Site.author site) with
  ...
  let items = List.map (fun (sec, page, renderer) ->
    ...
    let url = match (Page.get_key_as_string page "source") with
    | Some url -> Uri.of_string url
    | None -> Uri.with_uri ~path:(Some (Uri.path (Section.uri ~page sec))) (Site.uri site)
    in
    let link = Syndic.Atom.link ~title:(Page.title page) ~rel:Alternate url in

    let content: Syndic.Atom.content = Html(None, renderer page) in
    let updated = Page.date page in
    let authors = (author, []) in

    Syndic.Atom.entry ~id:url ~authors ~title ~updated ~published:updated ~content ~links:[link] ()
  ) (recent_pages pages) in

  let feed_link = Uri.with_uri ~path:(Some (Uri.path (Section.uri base_section))) (Site.uri site) in
  let feed_title: Syndic.Atom.text_construct = Text(Site.title site) in
  let generator = Syndic.Atom.generator ~uri:(Uri.of_string "https://github.com/mdales/webplats/") "webplats" in
  let updated = match pages with
  | (_, hd, _) ::_ -> Page.date hd
  | [] -> Ptime.epoch
  in

  let channel_uri = Uri.with_uri
    ~path:(Some (Uri.path (Section.uri base_section)))
    (Site.uri site) in
  let self_uri = Uri.with_uri
  ...
  let feed = Syndic.Atom.feed ~id:feed_link ~links ~authors:[author] ~title:feed_title ~updated ~generator items in
