...
let render_video filename thumb_opt looped =
    let looped_at = match looped with
    | true -> [At.true' "looped"]
    | false -> []
...
let render_audio filename =
    let html = El.div ~at:[At.class' "listimage"] [
        El.div [
            El.audio ~at:[At.true' "controls"] [
                El.source ~at:[
                    At.src filename;
                    At.type' "audio/mp4";
                ] ();
    ...
    let html = El.div ~at:[At.class' "listimage"] [
        El.div [
            El.iframe ~at:[
                At.width 560;
                At.height 315;
                At.src (Printf.sprintf "https://www.youtube-nocookie.com/embed/%s" code);
                At.title "YouTube video player";
                At.v "frameborder" "0";
                At.v "allow" "accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture";
...
let render_raster filename alt klass dims =
    ...
    let srcset = match (is_image_retina dims) with
    | true -> (
    ...
    let alttext = match alt with
    | Some desc -> [At.alt desc]
    | None -> []
    in

    let extraclass = match klass with
    | Some name -> [At.class' name]
    | None -> []
    in
...
let render_compare_rasters filename1 filename2 label1 label2 dims =
    ...
    let images = List.map (fun (filename, klass, label) ->
        let name, ext = Fpath.split_ext (Fpath.v filename) in
        let srcset = match (is_image_retina dims) with
        | true -> (
          let retina_filename = Printf.sprintf "%s@2x%s" (Fpath.to_string name) ext in
        ...
        (filename1, "before", label1);
        (filename2, "after", label2);
    ] in

    let html = El.div ~at:[At.class' "compare"; At.id identifier] (images @ [
        El.div ~at:[At.class' "handle"] [];
        El.span ~at:[At.class' "label"; At.class' "label-before"] [El.txt label1];
        El.span ~at:[At.class' "label"; At.class' "label-after"] [El.txt label2];
        El.input ~at:[
            At.type' "range";
            At.v "min" "0";
            At.v "max" "100";
    ...
    and script = El.script [El.unsafe_raw {|
...
let render_vector filename alt klass =
  let customclass = match klass with
  | Some name -> [At.class' name]
  | None -> []
  in
  let alttext = match alt with
...
let render_chart _style filename xaxis yaxis =
  ...
  let script = El.script [ El.unsafe_raw (Printf.sprintf {|
    var vlSpec = {
    ...
    vegaEmbed('#%s', vlSpec);
    |} filename xaxis yaxis identifier)] in
  [html ; script]
...
let render_geojson filename =
  ...
  fetch('%s')
    ...
    |} identifier identifier identifier filename identifier identifier)] in
    [html ; script]
