...
(** This function will be called when Webplats requires a thumbnail image for
    ...
    "titleimage" entry, but the renderer can decide to select whatever they like
    from the provided page. *)

type image_loader_t = Page.t -> string -> int * int -> Eio.Fs.dir_ty Eio.Path.t
(** This function will be called to render a given image within the page at a
    specified file size. *)

type diagram_loader_t = Page.t -> string -> Eio.Fs.dir_ty Eio.Path.t
...
type page_renderer_t =
  ...
  Page.t option ->
  Htmlit.El.html
(** A page renderer should take a page and return the full expanded HTML for
    that page *)

type meta_page_renderer_t = Page.t -> page_renderer_t
(** The meta page renderer takes a page and returns the function used to render
    it on demand. *)
          ...
          [
            {
              ...
              handler =
                render_static_file Eio.Path.(Page.path page / img.filename);
            };
          ]
      | _ -> (
          [
            {
              ...
              handler =
                deferred_render_static_file (fun _ ->
                    thumbnail_loader ~retina:false page);
            };
            {
              uri = Section.uri ~page ~resource:"thumbnail@2x.jpg" sec;
              handler =
                deferred_render_static_file (fun _ ->
                    thumbnail_loader ~retina:true page);
            };
            {
              uri = Section.uri ~page ~resource:"preview.jpg" sec;
              handler =
                deferred_render_static_file (fun _ ->
                    image_loader page img.filename (2048, 2048));
            };
          ]
          @
          ...
          | "photos" ->
              ...
              [
                {
                  ...
                  handler =
                    deferred_render_static_file (fun _ ->
                        image_loader page img.filename (1008, 800));
                };
                {
                  uri = Section.uri ~page ~resource:("scrn_" ^ retina_name) sec;
                  handler =
                    deferred_render_static_file (fun _ ->
                        image_loader page img.filename (2016, 1600));
                };
                {
              ...
              @ [
                  ...
                  {
                    uri =
                      Section.uri ~page ~resource:("album_" ^ img.filename) sec;
                    handler =
                      deferred_render_static_file (fun _ ->
                          image_loader page img.filename (300, 300));
                  };
                  {
                    uri =
                    ...
                    handler =
                      deferred_render_static_file (fun _ ->
                          image_loader page img.filename (600, 600));
                  };
                ]
          | _ -> []))
    ...
    (fun (i : Frontmatter.image) ->
      [
        (* non retina *)
        {
          ...
          handler =
            deferred_render_static_file (fun _ ->
                image_loader page i.filename (720, 1200));
        };
        (* retina *)
  ...
  [
    {
      ...
      handler =
        deferred_render_static_file (fun _ ->
            image_loader page filename (900, 700));
    };
    (let name, ext = Fpath.split_ext (Fpath.v filename) in
     let retina_name = Fpath.to_string name ^ "@2x" ^ ext in
     {
       ...
       handler =
         deferred_render_static_file (fun _ ->
             image_loader page filename (900 * 2, 700 * 2));
     });
  ]
          ...
          [
            {
              uri = Section.uri ~page ~resource:filename sec;
              handler =
                deferred_render_static_file (fun _ -> diagram_loader page code);
            };
          ]
      | _ -> [])
    (Page.shortcodes page)
            ...
            (fun alias ->
              {
                ...
                handler =
                  (fun _ ->
                    let headers =
                      Http.Header.of_list
                        [ ("Location", Uri.to_string (Section.uri ~page sec)) ]
                    in
      ...
      [
        {
          ...
          handler =
            (fun _ ->
              ...
              Cohttp_eio.Server.respond ~status:`Moved_permanently ~headers
                ~body:(Eio.Flow.string_source "")
                ());
        };
      ]
      ...
      (* Dream.log "Adding %s" (Uri.to_string (Section.uri ~page sec)); *)
      {
        ...
        handler =
          (fun _ ->
            ...
            let last_modified = Httphelper.ptime_to_last_modified mtime in
            let headers =
              Http.Header.of_list [ ("Last-modified", last_modified) ]
            in
            let body =
  ...
  [
    {
      ...
      handler =
        (fun _ ->
          ...
          let headers =
            Http.Header.of_list [ ("Content-Type", "application/rss+xml") ]
          in
          Cohttp_eio.Server.respond_string ~headers ~status:`OK ~body ());
    };
    {
...
let routes_for_section ~section_renderer ~page_renderer ~page_body_renderer
  ...
  Fmt.pr "%s@\n" (Uri.to_string (Section.uri sec));
  {
    ...
    handler =
      (fun _ ->
        let body =
          (section_renderer sec) site sec |> El.to_string ~doctype:true
        in
...
let rec inner_collect_static_routes src_path prefix =
  ...
  |> List.concat_map (fun name ->
      ...
      | true -> inner_collect_static_routes full_path url_path
      | false ->
          [
            {
...
let collect_static_routes site =
  ...
  let theme_static_dir =
    Eio.Path.(website_dir / "themes" / Site.hugo_theme site / "static")
  in

  let things_to_be_published =
    List.concat_map
      (fun static_dir -> inner_collect_static_routes static_dir "")
...
let routes_for_taxonomies ~taxonomy_renderer ~taxonomy_section_renderer
    ~page_renderer ~page_body_renderer ~thumbnail_loader ~image_loader
    ~diagram_loader site =
    ...
    (fun (name, taxonomy) ->
      {
        ...
        handler =
          (let render_taxonomy = taxonomy_renderer taxonomy in
           let body =
             render_taxonomy site taxonomy |> El.to_string ~doctype:true
           in
...
let of_site ~section_renderer ~taxonomy_renderer ~taxonomy_section_renderer
  ...
  let sections =
    List.concat_map
      (routes_for_section ~thumbnail_loader ~image_loader ~diagram_loader
         ~section_renderer ~page_renderer ~page_body_renderer site)
      (Site.sections site)
  in

  let taxonomies =
    routes_for_taxonomies ~thumbnail_loader ~image_loader ~diagram_loader
      ~taxonomy_renderer ~taxonomy_section_renderer ~page_renderer
    ...
    | Some digest_path ->
        [
          (let site_path = Site.path site in
           ...
           { uri = Uri.of_string ("/" ^ stem); handler = render_static_file te });
        ]
  in
